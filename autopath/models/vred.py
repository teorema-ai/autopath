                ...
                for k in range(self.vred.n_classes):
                    for s in range(self.vred.latent_gaussians.n_scales):
                        ...
                        for c in range(latent_variance.shape[0]):
                            ...
                            self.logger.experiment.add_scalar(f"latent_mix_variance_mean/component={k}/scale={s}/channel={c}", latent_variance[c].mean(), self.global_step)
...
class VariationalReEncoderDecoderStill(Datablock):
    ...
    def __build__(self):
        logger = L.pytorch.loggers.TensorBoardLogger(save_dir=self.dirpath('logs'), default_hp_metric=False, name=self.tag)
