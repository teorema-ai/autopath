...
class FeaturesPairwiseDistancesChunk(Datablock):
    ...
    def __build__(self, features):
        ...
        M = features.shape[0]
        rows = rng.permutation(M)
        _rows = rows[self.cfg.chunk_idx*self.cfg.row_chunk_size:min((self.cfg.chunk_idx+1)*self.cfg.row_chunk_size, M)]
        del rows
        cols = rng.permutation(M)
        col_offset = rng.integers(M-self.cfg.col_chunk_size)
        _cols = cols[col_offset:min(col_offset+self.cfg.col_chunk_size, M)]
        del cols
        self.log.detailed(f"Building FeaturePairwiseDistancesChunk at index {self.cfg.chunk_idx} with rows {_rows} and {len(_cols)} columns on device {features.device}")
        pairwise_distances = torch.cdist(features[_rows], features[_cols]).to('cpu')
        self.log.debug(f"Built FeatureDistanceChunk at index {self.cfg.chunk_idx} with shape {pairwise_distances.shape} on device {features.device}")
        write_npz(self.path('rows', ensure_dirpath=True), rows=_rows)
        write_npz(self.path('cols', ensure_dirpath=True), cols=_cols)
        write_tensor(pairwise_distances, self.path('distances', ensure_dirpath=True))
        del _rows
...
class FeaturesPairwiseDistances(Datablock):
    ...
    def features(self):
        self.log.debug(f"Reading features from {len(self.cfg.featurebags.bags)} feature bags")
        feature_list = []
        for featurebag in self.cfg.featurebags.bags:
    ...
    def _chunks(self, shape):
        assert self.cfg.n_chunks*self.cfg.row_chunk_size <= shape[0], f"Too many chunks or chunks too big for n_features: {shape[0]}"
        assert self.cfg.col_chunk_size <= shape[0], f"Too row_chunk_size too large for n_features {shape[0]}"
        return [FeaturesPairwiseDistancesChunk(spec=dict(
                    featurebags=self.spec['featurebags'],
                    chunk_idx=i,
                    row_chunk_size=self.cfg.row_chunk_size,
                    col_chunk_size=self.cfg.col_chunk_size,
                    seed=self.cfg.seed,
                    )) 
    ...
    def __build__(self):
        ...
        self.log.debug(f"Forming {self.cfg.n_chunks} FeaturesPairwiseDistancesChunks from features of shape {features.shape}, "
                       f"row_chunk_size {self.cfg.row_chunk_size}, col_chunk_size: {self.cfg.col_chunk_size}"
        )
