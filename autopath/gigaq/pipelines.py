...
def gigapath_bipolar_feature_bags_probe(name, n_bins: int = 2, use_gpu: bool = False, gpu_batch_size: int = None) -> LogisticFeatureBagProbe:
    return BipolarFeatureBagProbe(spec=dict(featurebagclip=gigapath_feature_bag_clip(name), n_bins=n_bins,), use_gpu=use_gpu, gpu_batch_size=gpu_batch_size)
