...
# dbx "autopath.gigaq.pipelines.gigapath_features_pairwise_distances('GIGAPATH_BASELINE_CPTAC_8020_TEST_4000', n_gpus=4).build()"
def gigapath_features_pairwise_distances(name, sideband_layer: Optional[str] = None, n_gpus: int = 1) -> FeaturesPairwiseDistances:
...
def gigapath_features_sorted_distances(name, n_workers: int = 1, use_gpus: bool = False) -> FeaturesSortedDistances:
    ...
    if name == "GIGAPATH_BASELINE_CPTAC_8020_TEST_4000_25_05_005":
        return FeaturesSortedDistances(
                    spec=dict(featurebags_pairwise_distances=f"$autopath.gigaq.pipelines.gigapath_features_pairwise_distances('GIGAPATH_BASELINE_CPTAC_8020_TEST_4000')",
                              max_n_chunks=25,
